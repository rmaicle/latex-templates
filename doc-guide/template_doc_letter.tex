...
% TODO 20250831: What does this do?
\usepackage{ifdraft}
\usepackage{titlesec}
\titlelabel{\thetitle\; }
\usepackage{ragged2e}
...
% Draw vertical lines as guidelines
$if(showframe)$
\usepackage{eso-pic}
\AddToShipoutPictureBG{%
  \AtPageLowerLeft{%
    \color{gray!50}%
    % Example: Draw two vertical lines (left/right margins)
    % \rule{4.0pt}{\paperheight} % at left edge
    \hspace{1.70in}\rule{0.4pt}{\paperheight} % at 210pt from left
    % \hspace{210pt}\rule{0.4pt}{\paperheight} % another guide
  }%
}
$endif$
...
$if(listings)$
    ...
    \ifpdftex
        \lstdefinestyle{terminal}{
            ...
            % Sets the width of a single character box for fixed and
            % flexible column mode (both to the same value or
            % individually).
    ...
    \ifpdftex
        ...
        \lst@AddToHook{TextStyle}{%
            ...
            \ttfamily\relscale{2.0}}
        \makeatother
    \fi
    \ifxetex
        \makeatletter
        \lst@AddToHook{TextStyle}{%
            \let\lstinline@basicstyle\inlinettfont\selectfont}
        \makeatother
    ...
    \newcommand{\codesectwo}[1]{{\relsize{+0.99}\texttt{#1}}}
    \newcommand{\codesecthree}[1]{{\relsize{+0.96}\texttt{#1}}}
    \newcommand{\codesecfour}[1]{{\relsize{+0.91}\texttt{#1}}}
    \newcommand{\codesecfive}[1]{{\relsize{+0.84}\texttt{#1}}}
$endif$
...
% Define a command to make side boxes with color and thickness and an
...
\newcommand{\sidebox}[4]{%
    ...
    \newtcolorbox{#1}[1][]{enhanced,
        ...
        boxrule=0pt,
        left=3mm,
        borderline west={#3}{0pt}{#2},
        fonttitle=\bfseries,
%        coltitle=black,
%        % Only insert title if #2 is not empty:
%        title={\ifstrempty{#2}{##1}{#2\space\space\ ##1}},
%        % Avoid extra space if title is empty:
%        before title={\ifstrempty{#2}{}{\par\vspace{1.5mm}}},
%        colbacktitle={\ifstrempty{#2}{}{#3!10}}
    }%
}

% Define concrete environments
\sidebox{iside}{cyan!80!black}{3pt}
...
% #4 = bar thickness
    \newtcolorbox{#1}[1][]{enhanced,
        ...
        title={#2\space\space ##1}}%
}

% Define concrete environments
% TODO: Show examples of how to use here
\sideboxwithicon{itodo}{\faClipboardCheck}{cyan!80!black}{3pt}
\sideboxwithicon{inote}{\faInfoCircle}{blue!60!black}{3pt}
\sideboxwithicon{itip}{\faLightbulb}{yellow!90!black}{3pt}
\sideboxwithicon{iimportant}{\faExclamationCircle}{green!60!black}{3pt}
\sideboxwithicon{iwarning}{\faExclamationTriangle}{orange!90!black}{3pt}
\sideboxwithicon{icaution}{\faSkullCrossbones}{red!70!black}{3pt}
\sideboxwithicon{ipitfall}{\faTimesCircle}{red!80!black}{3pt}
...
$if(title)$
    ...
    \begin{flushright}
        ...
        $if(doctype)$
            {\normalsizerrr\textnormal{$doctype$}}\\[0.2em]
        $endif$
        $if(version)$
            ...
            {\normalsizerrr\textnormal{\ifoptiondraft{Draft}{}\space $version$}}\\[0.2em]
            % \vspace*{0.5em}
        $endif$
        {\normalsizerr\textnormal{$date$}}\\[0.2em]
        \vspace*{1.75in}
        $if(author)$
            $if(author_heading)$
                {\normalsizerr\textnormal{$author_heading$}}\\[1em]
            $endif$
            $for(author)$
                $if(author.name)$
                    \vspace*{1.4em}
                    {\normalsizerr\textbf{$author.name$}}\\[0.2em]
                    $if(author.title)$
                        {\normalsizerr\textnormal{$author.title$}}\\[0.2em]
                    $endif$
                    $if(author.company)$
                        {\normalsizerr\textnormal{$author.company$}}\\[0.2em]
                    $endif$
                    $if(author.email)$
                        {\normalsizerr\textnormal{$author.email$}}\\
                    $endif$
                $endif$
            $endfor$
        $endif$
        ...
        \vspace{\fill}
        $if(distribution)$
            {\normalsize\textnormal{$distribution$}}
        $endif$
    \end{flushright}
$endif$
...
$if(with_copyright)$
    ...
    \begin{flushleft}
        ...
        \hfill\newline
        $if(version)$
            {\normalsize\textnormal{\ifoptiondraft{Draft}{}\space $version$}}
        $endif$
        \hfill\newline
        $if(author)$
            \normalsize by
...
\fancyhead[C]{}
\fancyhead[R]{\thepage}
% Add vertical spacing for the header text and header rule
\makeatletter
\renewcommand{\headrule}{%
  \vspace{4pt} % extra space before the rule
  \hrule height 0.4pt % thickness of the rule
  % \vspace{-2pt} % adjust spacing after the rule if needed
}
\makeatother
% Header and body text vertical space
\setlength{\headsep}{20pt} % default ~12pt
