...
$if(title)$
    ...
    \begin{flushright}
        ...
        \vspace*{\fill}
        $if(distribution)$
            {\fontsize{11pt}{13.2pt}\usefont{T1}{bch}{m}{n}$distribution$\par}
        $endif$
    \end{flushright}
$endif$
...
\begin{description}
\item[\Terminalfont\fontsize{10}{10}\selectfont{Constant width (Latin Modern Typewriter)}]\hfill \\
Used for terminal command texts and terminal output texts.

\item[\BeraMonofont\fontsize{10}{10}\selectfont{Constant width (Bera Mono)}]\hfill \\
Used for source code listings and command syntaxes.

\item[\ding{224} \BeraMonofont\fontsize{10}{10}\itshape\selectfont{note (Bera Mono)}]\hfill \\
Used in terminal command, output texts, source code listings and
command syntaxes to display additional information or to emphasize a
line that requires the reader's attention.
\end{description}
